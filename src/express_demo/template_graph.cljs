...
(defn add-template-entry [g t reg]
  (let [parent (:module-name t)
        possible-children (template-invocations->context-pairs t)
        registered-children (map (fn [child-pair]
                                   (into [] (filter #(identity true) child-pair))
                                   ;; (into [] (filter #(contains? reg %) child-pair))
                                   )
                                 possible-children)
        child-edges (into [] (map (fn [child-pair]
                                    (match [child-pair]
    ...
    (println `[g parent ~@(apply concat child-edges)])
